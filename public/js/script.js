...
function showEventsForDate(dateStr) {
  ...
  selected.forEach((ev) => {
    list.innerHTML += `
      <div class="card mb-3 available-event-card">
        <img src="${ev.image}" class="card-img-top" alt="${ev.title}">
        <div class="card-body">
          <h5 class="card-title">${ev.title}</h5>
          <p class="card-text">${ev.location}</p>
          <a href="event-booking.html?event=${encodeURIComponent(ev.title)}&date=${ev.date}" class="btn btn-sm btn-primary">Book Now</a>
        </div>
      </div>`;
  });
